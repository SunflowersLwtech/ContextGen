...
| 因素 | 说明 |
|------|------|
| Vertex AI Memory Bank 底层 | Memory Bank 内部使用 Firestore，技术栈统一 |
...
> **注意**：如果使用 Vertex AI Memory Bank（首选方案），此集合**不需要手动创建**。Memory Bank 自动管理记忆存储和检索。仅当 Memory Bank 提取逻辑不够灵活、需要切换到自建方案时才使用。
...
| 服务 | 管理的数据 | 底层存储 | 集成代码量 |
|------|----------|---------|-----------|
| **Vertex AI Memory Bank** | 长期记忆（提取的事实/偏好/关系） | 内部 Firestore（`agent_engine_id` 隔离） | ~30 行 |
...
│  │  └─ face_library/    │                                           │
│  │      - person_name   │   ┌───────────────────────┐               │
│  │      - embedding(512)│   │ [Auto] Memory Bank    │               │
│  │      - relationship  │   │  内部 Firestore 集合   │  ← 首选方案  │
│  │                      │   │  (agent_engine_id)    │               │
│  └──────────────────────┘   └───────────────────────┘               │
│                                                                     │
│                             ┌───────────────────────┐               │
...
数据消费关系:
  ...
  Memory Bank (auto)      → PreloadMemoryTool (每轮自动加载)
...
# === Memory Bank ===
AGENT_ENGINE_ID=<auto-generated>               # Memory Bank Agent Engine ID
...
| **Agent Engine 创建** | Python SDK `client.agent_engines.create()` | 100% | Vertex AI API |
...
Phase C: AI 执行 — MCP + Vibe Coding (1-2 hr)
  ...
  ├── C6. Python SDK 创建 Agent Engine (Memory Bank)
...
| Vertex AI Memory Bank | 公开预览免费 | — | $0 |
| **合计** | | | **~$37-45** |
...
| Vertex AI Memory Bank 配额限制 | 记忆存储受限 | Mem0 作为 fallback |
