...
* **数据源**：当前视频帧、音频打断信号、手机加速度计（运动状态/步频）、手机麦克风（环境噪声 dB）、手表心率（可选）。所有 Core 数据源来自手机，无需额外硬件。
* **处理逻辑**：拥有最高中断优先级 (Interrupt Priority)。如果心率可用且触发恐慌信号，Orchestrator 会直接清空当前的 TTS 播放队列，降级 LOD，并插播警告。即使无手表，运动状态突变（突然停下/加速）同样触发 LOD 调整。

### **B. 会话期上下文 (Session Context) \- 分钟级到小时级**

* **数据源**：用户今天出行的目的（”我要去面试”）、当前所在的空间类型（室内/室外/车上）、近 30 分钟的步频均值、时间上下文（时段推断：早通勤/工作时间/晚间）。
...
3. **”Phone-First Sensing” (手机优先的轻量感知)**：
   * “我们刻意不把用户变成’钢铁侠’。SightLine 的核心 Context 信号——运动状态、步频、环境噪声、GPS、时间——全部来自用户手里的手机。零额外设备即可驱动完整的自适应 LOD 系统。智能手表的心率只是可选增强。结合时空 Context，系统能学习到用户每次经过某个十字路口时步频都会紊乱。下次接近该路口时，无需用户开口，AI 会提前切入高精度的导航引导。”