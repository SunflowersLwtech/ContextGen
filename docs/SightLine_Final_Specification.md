...
|---------|------|---------------|-----------|
| 视频流 | 手机摄像头 | `getUserMedia()` → JPEG 768x768 @ 1FPS → WebSocket 上传 | 智能眼镜、胸前摄像头 via SEP-Vision |
| 音频 | 手机麦克风 | `getUserMedia()` → AudioWorklet resample 16kHz PCM → WebSocket 上传 | 骨传导耳机、助听器 via SEP-Audio |
| **运动状态** | 手机加速度计 | Developer Console 模拟注入 | iOS CMMotionActivityManager / Android ActivityRecognitionClient |
| **步频** | 手机加速度计 | Developer Console 滑块模拟注入 | iOS CMPedometer / Android TYPE_STEP_COUNTER |
| **环境噪声** | 手机麦克风 | Developer Console 滑块模拟注入 | 麦克风背景 RMS → dB 实时计算 |
| **时间上下文** | 手机时钟 | 自动获取 (无需模拟) | 同左 + 结合长期记忆推断时段模式 |
| 地理位置 | 手机 GPS | 手机 GPS (真实) | 同左 + 更精确的 UWB 定位 |
| 心率 *(可选)* | 智能手表 | Developer Console 滑块模拟注入 | Apple Watch / 运动手环 via SEP-Telemetry |
...
Google Live API 仅支持 WebSocket (WSS)，不支持 WebRTC/gRPC。前端通过 `getUserMedia()` 采集媒体，经 WebSocket 发送到 Cloud Run 后端，后端通过 ADK `run_live()` + `LiveRequestQueue` 管理与 Gemini 的双向流式会话。
...
│  ├── getUserMedia() → 摄像头 (768x768 JPEG, 1FPS) + 麦克风               │
│  ├── AudioWorklet: pcm-recorder-processor (44.1kHz→16kHz PCM resample)   │
│  ├── WebSocket client → ws://cloud-run-url/ws/{user_id}/{session_id}     │
│  ├── AudioWorklet: pcm-player-processor (24kHz PCM playback)             │
│  ├── Telemetry: Developer Console 滑块 → JSON via 同一 WebSocket          │
│  └── Wake Lock API (防息屏)                                               │
└────────────────────────────┬───────────────────────────────────────────────┘
                             │ WebSocket (WSS)
                             ▼
...
│  │                                                                        │
│  ├── Session: InMemorySessionService (Hackathon 足够)                     │
...
from google.genai.types import (
    ...
    ProactiveAudio, AffectiveDialog,
)
...
runner = Runner(agent=orchestrator, session_service=InMemorySessionService(),
                app_name="sightline", agent_name="sightline_orchestrator")
...
async def websocket_endpoint(ws: WebSocket, session_id: str):
    ...
    # RunConfig — 含 Session Resumption + Context Compression
    run_config = RunConfig(
        ...
        session_resumption_config=SessionResumptionConfig(
            handle=session_handles.get(session_id),   # 首次为 None，重连时带上
        ),
        context_window_compression_config=ContextWindowCompressionConfig(
            sliding_window=SlidingWindow(),            # 自动压缩，突破 2min 限制
        ),
    ...
    live_events = runner.run_live(
        session=session, live_connect_config=run_config,
    )
    live_request_queue = live_events.input
    ...
    async def upstream():
        async for raw in ws.iter_bytes():
            ...
            if msg.type == "audio":
                live_request_queue.send_audio(msg.data)       # PCM 16kHz mono
            elif msg.type == "video":
                live_request_queue.send_image(msg.data)       # JPEG frame
            elif msg.type == "telemetry":
                # Telemetry 走同一 WebSocket，以 text 方式注入上下文
                live_request_queue.send_content(
                    f"[TELEMETRY UPDATE] {json.dumps(msg.data)}",
                    behavior="SILENT"
                )
    ...
    async def downstream():
        nonlocal session_handles
        async for event in live_events:
            if event.is_audio:
                await ws.send_bytes(event.audio_chunk)        # PCM 24kHz mono
            elif event.go_away:
                # GoAway 信号：Gemini 即将断连，缓存 handle 准备重连
                session_handles[session_id] = event.resumption_handle
                await ws.send_json({"type": "go_away", "retry_ms": 500})
            elif event.session_resumption_update:
                # 定期更新 handle（即使未断连也要缓存最新的）
...
| **Audio Pipeline** | AudioWorklet (`pcm-recorder-processor` / `pcm-player-processor`) | 浏览器 44.1kHz→16kHz PCM 采集、24kHz PCM 播放 |
...
| **Frontend** | React PWA (Vite, `getUserMedia` + WebSocket) | 手机摄像头/麦克风接入，`vite-plugin-pwa` |
...
│   └── face_id_agent.py       # 人脸库匹配 (InsightFace buffalo_l)
├── tools/
...
│   ├── src/
│   │   ├── App.tsx            # 极简单按钮 UI
│   │   ├── WebSocketClient.ts # WebSocket 音视频连接（非 WebRTC）
│   │   ├── TelemetryConsole.tsx  # Developer Console (模拟器)
│   │   └── LODIndicator.tsx   # LOD 状态色彩指示
│   ├── public/
│   │   └── audio_worklets/
│   │       ├── pcm-recorder-processor.js   # AudioWorklet: getUserMedia 44.1kHz → 16kHz PCM 重采样
│   │       └── pcm-player-processor.js     # AudioWorklet: 24kHz PCM → AudioContext 播放
│   └── package.json
├── infrastructure/
│   ├── Dockerfile
...
| **iOS PWA standalone 模式 `getUserMedia` 不可用** | **中** | 不使用 `display: standalone`；引导用户从 Safari 直接使用（或 Add to Home Screen 后保持 `display: browser`）|
...
| 优先级 | 功能 | 说明 |
|--------|------|------|
| **P0 — 必须有** | LOD Engine (3 级) + Orchestrator + Vision Agent + 前端 PWA (getUserMedia + WebSocket) | 没有这些就没有产品 |
