...
【Orchestrator Agent】←── Adaptive LOD Engine
  │  (Gemini 2.5 Flash — 快速路由)
  │
  ├──▶ [Vision Sub-Agent]     Gemini 2.5 Pro — 帧解析、场景描述、表情识别
  ├──▶ [OCR Sub-Agent]        Gemini 2.5 Flash — 文字/标签/菜单/药品读取
  ├──▶ [Navigation Sub-Agent] Google Maps API + Geocoding — 位置、方向、POI
  ├──▶ [Memory Sub-Agent]     Firestore — 跨会话偏好读写、人脸库查询
  ├──▶ [Face ID Sub-Agent]    face_recognition + Firestore — 人脸匹配
  └──▶ [Grounding Sub-Agent]  Google Search — 验证品牌/产品/地点信息
```
...
async def process_input(input_stream, telemetry):
    ...
    if intent == "where_am_i":
        geo_ctx = await NavigationAgent.locate()
...
|-------|-----------|---------|
| **AI Model** | Gemini 2.5 Flash (Orchestrator) + Gemini 2.5 Pro (Vision) | 快速路由 + 深度视觉理解 |
| **Agent Framework** | Google ADK (Python) | Multi-agent 编排 |
| **Live Streaming** | Gemini Live API (WebSocket) | 实时双向音视频 |
| **Face Recognition** | `face_recognition` / DeepFace | 人脸库匹配 (独立 pipeline) |
...
│   ├── navigation_agent.py    # 地理位置 + POI
│   ├── memory_agent.py        # 跨会话记忆读写
│   ├── face_id_agent.py       # 人脸库匹配 (face_recognition)
│   └── grounding_agent.py     # Google Search 事实验证
├── lod/
...
- [ ] OCR Sub-Agent (文字读取)
- [ ] Navigation Sub-Agent (Google Maps 集成)
...
| **P1 — 强烈需要** | OCR Agent + Google Search Grounding | Demo 中需要读文字和验证信息 |
