...
根据用户 `vision_status` 动态调整 LOD 描述策略：

| 用户类型 | 描述策略 |
|---------|----------|
| **先天盲人** (congenital_blind) | 使用触觉、空间、声音类比描述。**不描述颜色**。用尺寸比较 ("像背包那么大") 代替视觉描述。 |
| **后天盲人** (acquired_blind) | 可以使用颜色和视觉记忆相关的词汇。用户可能记得视觉概念。 |
...
```python
# 先天盲人版本
if user_profile.vision_status == "congenital_blind":
    lod_context += """
...
- 自动连接 WebSocket → Cloud Run → Gemini Live API
- 自动开启麦克风 + 摄像头 (`getUserMedia`)
- 自动防息屏 (Wake Lock API)
- 系统的 Proactive Audio 自动决定何时说话，中央呼吸灯效果指示状态

**技术实现**：
- 双击打断 = 发送 `activity_start` 信号触发 Gemini barge-in
- 上/下滑 = 注入 Telemetry: `{"user_gesture": "lod_up/lod_down"}`
- 触觉反馈 = `navigator.vibrate()` API (Android 原生支持; iOS PWA 不支持振动，降级为音效)

### 7.3 Affective Dialog 配置

利用 Gemini Live API 的 Affective Dialog 能力 (已 GA, Aug 2025)：
